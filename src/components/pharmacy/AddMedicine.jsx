...
const AddMedicine = () => {
  ...
  const [medicineData, setMedicineData] = useState(initialFormState);
  const [suppliers, setSuppliers] = useState([]); // ADDED: State to hold suppliers
  const [submitting, setSubmitting] = useState(false);
  const [successMessage, setSuccessMessage] = useState('');
  const [errorMessage, setErrorMessage] = useState('');
  
  // ADDED: useEffect to fetch suppliers when the component loads
  useEffect(() => {
    const fetchSuppliers = async () => {
      try {
        ...
        setErrorMessage("Could not load supplier list.");
      }
    };
    fetchSuppliers();
  }, []);
  ...
  batch_number: medicineData.batchNumber,
    ...
    try {
      // UPDATED: Using the consistent apiClient
      const response = await apiClient.post('/api/pharmacy/medicines', payload);
    ...
    <div className="p-6">
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          {/* --- UPDATED: This dropdown is now dynamic --- */}
          <Select label="Supplier/Manufacturer" value={medicineData.manufacturer} onChange={(e) => handleChange('manufacturer', e.target.value)} required>
            <option value="">Select Supplier</option>

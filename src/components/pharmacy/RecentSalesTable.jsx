...
//       try {
//         const response = await apiClient.get('/api/pharmacy-invoices');
//         // CHANGED: Slice the top 3 most recent invoices instead of 5
//         const recentInvoices = response.data.slice(0, 3);

//         const formattedSales = recentInvoices.map(invoice => ({
//           id: `...${invoice._id.slice(-6)}`,
//           name: invoice.items.map(item => item.description).join(', '),
...
//                     <td className="px-6 py-4 truncate max-w-xs">{sale.name}</td>
...
const RecentSalesTable = () => {
  ...
  useEffect(() => {
    const fetchRecentSales = async () => {
      try {
        const response = await apiClient.get('/api/pharmacy-invoices');
        const recentInvoices = response.data.slice(0, 3); // Get top 3

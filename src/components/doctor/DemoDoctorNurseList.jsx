...
const Icons = {
  ...
  Login: () => <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h7a3 3 0 013 3v1" /></svg>,
  Check: () => <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
};

const DemoDoctorNurseList = () => {
  ...
  const [demoLoginLoading, setDemoLoginLoading] = useState(null); // Track which staff is being logged in
  ...
  useEffect(() => {
    // Check if demo user from separate localStorage flag
    const demoFlag = localStorage.getItem('isDemoUser') === 'true';
    setIsDemoUser(demoFlag);
    
    // You can also get the role if needed
    const userData = JSON.parse(localStorage.getItem('hospitalUser') || '{}');
    console.log('Current user role:', userData.role, 'Is demo:', demoFlag);
  }, []);
  ...
  // Demo Login Handler - Updated to match main login functionality
  const handleDemoLogin = async (staffMember) => {
    setDemoLoginLoading(staffMember._id);
    try {
      const token = JSON.parse(localStorage.getItem('hospitalUser') || '{}').token;
      ...
      // Store IDs in localStorage based on role (same as main login)
      ...
      // Handle role verification for staff/nurse (same as main login)
      let finalRole = role;
  ...
  return (
    <div className="p-2 min-h-screen bg-slate-50/50 font-sans text-slate-800">
      ...
      {/* Demo Mode Banner - Shows if isDemoUser flag is true */}
      {isDemoUser && (
        <div className="mb-6 bg-purple-50 border border-purple-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-purple-600">Click the login button to access any staff account</p>
          </div>
        </div>
      )}
                            ...
                            <tr key={member._id} className="hover:bg-slate-50/80 transition-colors group">
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        ...
                                        <button onClick={() => navigate(`/dashboard/demo/doctor-profile/${member._id}`)} className="p-2 text-slate-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-colors" title="View Profile">
                                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                        </button>
                                        <button onClick={() => navigate(`/dashboard/demo/edit-doctor/${member._id}`)} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                                            <Icons.Edit />
                                        </button>
...
export default DemoDoctorNurseList;